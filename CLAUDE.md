...
**Nothing has been posted** — `GCam.Posts` is an empty project, and that is the last step of the first vertical slice. A 2D contour operation does generate a toolpath from selected edges, draw it, and save it inside the part. `docs/architecture.md` opens with the status table — trust it over any impression of progress, including the one this file gives.
...
- **Never set `IPropertyManagerPageControl.Visible` on a page you are about to show.** It kills SOLIDWORKS outright — silently, with nothing in any log, and only after about the fourth show, which makes it look like anything but what it is. Pages are therefore rebuilt for every show, with controls created at the visibility they need via `AddControl2`'s options. Populate from `LoadControls` before `Show2`, never `AfterActivation`, and keep control ids unique per page — duplicates are accepted in silence. **Writing to a shown page is per-call knowledge, not a blanket rule.** `Combobox.Clear` and `Combobox.InsertItem` each kill SOLIDWORKS on their first call. `Label.Caption`, `Visible`, `SetSelectionFocus` and `IPropertyManagerPage2.SetFocus` are all fine from an ordinary callback — the caption's reputation came from being bisected inside a handler that had put a modal WPF window over the page first. `docs/solidworks-api/property-manager-pages.md` has the measured table; add to it rather than generalising from it. A control whose contents genuinely cannot be written — a combobox's item list — needs `RebuildAfterHandlerReturns`, which closes and re-shows the page on a later turn of the message pump.
