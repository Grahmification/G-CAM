...
- **Never set `IPropertyManagerPageControl.Visible` on a page you are about to show.** It kills SOLIDWORKS outright — silently, with nothing in any log, and only after about the fourth show, which makes it look like anything but what it is. Pages are therefore rebuilt for every show, with controls created at the visibility they need via `AddControl2`'s options. Populate from `LoadControls` before `Show2`, never `AfterActivation`, and keep control ids unique per page — duplicates are accepted in silence. **Treat a shown page's controls as read-only altogether.** `Combobox.Clear`, `Combobox.InsertItem` and `Label.Caption` have each killed SOLIDWORKS on their first call, the same way `Visible` does, and no write to a shown page has yet been seen to survive. To change what a page displays, change the model and call `RebuildAfterHandlerReturns` — it closes and re-shows the page on a later turn of the message pump, which is safe precisely because the page is rebuilt for every show anyway.
